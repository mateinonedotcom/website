<script lang="ts">
	...
	import { ColorPalettes } from '$lib/color-palette';
	...
	export let colorPalette = ColorPalettes.default;
...
<BoardComponent let:square>
	<SquareComponent {colorPalette} {square}>
		...
		>
			{#if isMoveDestinationValid}
				<div
					class="{colorPalette.squareActive
						.bg} absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full h-full"
				></div>
			{/if}

			{#if null !== piece}
				<div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
					<PieceComponent {piece} {colorPalette} />
				</div>
			{/if}
		</div>
	</SquareComponent>
