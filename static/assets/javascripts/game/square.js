(function (window) {
    ...
    window.mateinone.game.Square = {
        addPiece: (square, pieceElement) =>
            getElement(square).appendChild(pieceElement)
        ,
        ...
        removeAllValidMoveHighlights: () => {
            document
                .querySelectorAll(`.${validMoveClass}`)
                .forEach(square => {
        ...
        addAllClickEventListeners: (squares, handler) =>
            squares.forEach(square => getElement(square).addEventListener("click", () => handler(square)))
        ,
